...
public class RNTrackPlayer: RCTEventEmitter {
    ...
    public func setupPlayer(config: [String: Any], resolve: RCTPromiseResolveBlock, reject: RCTPromiseRejectBlock) {
        ...
        player.event.playbackEndWithInfo.addListener(self) { [weak self] event in
            guard let self = self else { return }

            if event.reason == .playedUntilEnd && event.nextItem == nil {
                self.sendEvent(withName: "playback-queue-ended", body: [
                    "track": (event.currentItem as? Track)?.id as Any,
                    "position": event.currentTime,
                    ])
            } else if event.reason == .playedUntilEnd {
                self.sendEvent(withName: "playback-track-changed", body: [
                    "track": (event.currentItem as? Track)?.id as Any,
                    "position": event.currentTime,
                    "nextTrack": (event.nextItem as? Track)?.id as Any,
                    ])
            }
        }
